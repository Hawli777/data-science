...
pp1$CreditGrade <- ordered(pp1$CreditGrade,levels = c('AA', 'A', 'B', 'C', 'D', 'E', 'HR','NC'))
...
ggplot(aes(x =ListingCategory..numeric. ), data = pp) + 
  geom_bar(color = 'black', fill = '#FFBF00')+
...
ggplot(aes(x = LoanOriginalAmount), data = pp) + 
  geom_histogram(binwidth = 1000,color = 'black', fill = '#5858FA')
```

由上图看出，贷款金额多数集中在$10000以内。
...
ggplot(aes(x = Term), data = pp) + 
  geom_bar(color = 'black', fill = '#58FA58')
```
...
ggplot(aes(x = BorrowerState), data = pp) + 
  geom_bar(color = 'black', fill = '#0000FF')
...
ggplot(aes(x = IncomeRange), data = subset(pp,IncomeVerifiable = True)) + 
  geom_bar(color = 'black', fill = '#F7FE2E')
...
ggplot(aes(x = EmploymentStatus), data = subset(pp, !is.na(EmploymentStatusDuration))) + 
...
ggplot(aes(x = ProsperScore), data = pp2) + 
  geom_bar( color = 'black', fill = '#B40404')
```
...
ggplot(aes(LoanOriginalAmount,BorrowerRate,color = CreditGrade),data = subset(pp1,CreditGrade != 'NC')) +
  geom_point() +
